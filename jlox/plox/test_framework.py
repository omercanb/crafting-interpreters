...
class TestRunner:
    def __init__(self, lox_file: str, classpath: str):
    ...
    def run_with_print(self):
        result = subprocess.run(
            ["java", "-cp", self.classpath, "lox.Lox", self.lox_file, "--print"],
            capture_output=True,
            text=True,
        )
...
class LoxTestFramework:
    def __init__(self, test_files_dir: str, classpath: str):
    ...
    def run_test(self, lox_file: str):
        runner = TestRunner(lox_file, self.classpath)
...
if __name__ == "__main__":
    ...
    classpath = sys.argv[2] if len(sys.argv) > 2 else "../lox/lox"

    LoxTestFramework(test_dir, classpath).run_all_tests()
